...
** Notation
It will be helpful to have some notation, to make it easier to reason about categories.
#+begin_src agda2
_[_,_] : ∀ {i j} (C : Precategory i j) → (x y : C .ob) → Type j
C [ x , y ] = C .hom x y

seq : ∀ {i j} (∁ : Precategory i j) {x y z : ∁ .ob}
      (g : ∁ [ y , z ]) (f : ∁ [ x , y ])
      → ∁ [ x , z ]
seq ∁ g f = ∁ .comp g f

infix 15 seq
syntax seq C g f = g o⟨ C ⟩ f
#+end_src

* Categories
A category, in the sense of the HoTT book is on where the map "id-to-iso" is an equivalence. These are sometimes referred to as "univalent categories".
