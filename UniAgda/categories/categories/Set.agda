...
id-equiv-to-prop-on-type : ∀ {i j} (X X' : Type i) (Q : Type i → Type j)
              ...
              → (a : Q X) → (b : Q X')
              → (X ≡ X') ≃ ((X , a) ≡ (X' , b))
id-equiv-to-prop-on-type X X' Q F a b = (thm2-7-2 (X , a) (X' , b) ^ᵉ) oₑ
...
UA-for-sets : ∀ {i} (X X' : Set_ i)
              → (X ≡ X') ≃ (pr₁ X ≃ pr₁ X')
UA-for-sets {i} (X₁ , a) (X₂ , b) = (id-equiv-to-prop-on-type X₁ X₂ isSet (λ A → isSet-is-prop A) a b oₑ (univalence ^ᵉ)) ^ᵉ
...
set-equiv-iso-qinv {i} (X , a) (X' , b) = equiv-adjointify ((λ { (f , g , α , β) → f , (g , ((funext α) , (funext β)))}) ,
                   ...
                   λ { (f , g , α , β) → path-equiv-sigma _ _ (refl , (transport (λ F → (g , happly (funext α) , F) ≡ (g , α , β)) (pr₁ (pr₂ (pr₂ happly-isEquiv)) β ^)
...
qinv-of-sets-is-prop : ∀ {i} {A B : Set_ i} (f : pr₁ A → pr₁ B)
                       → isProp (qinv f)
qinv-of-sets-is-prop {i} {X , a} {Y , b} f (g , α , β) (g' , α' , β') = path-equiv-sigma _ _ ((funext (λ y → β' (g y) ^ ∘ ap g' (α y))) ,
                     path-equiv-sigma _ _ ((funextD (λ y → b _ _ _ _)) , funextD λ x → a _ _ _ _))
...
id-to-iso-equality {i} (X , a) (X' , b) = funext λ { refl → path-equiv-sigma _ _ ((funext (λ x → refl)) ,
