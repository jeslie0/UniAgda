...
private
  hom₃ : {i j : Level} {A : Type i} {P : A → Type j}
         {w w' : Σ[ x ∈ A ] (P x)}
         → path-sigma w w' o path-equiv-sigma w w' ~ id
  hom₃ {w = _ , _} {w' = _ , _} (refl , refl) =  refl

  hom₄ : {i j : Level} {A : Type i} {P : A → Type j}
         (w w' : Σ[ x ∈ A ] (P x))
         → path-equiv-sigma w w' o path-sigma w w' ~ id
  hom₄ (a , b) .(a , b) refl = refl

thm2-7-2 : {i j : Level} {A : Type i} {P : A → Type j}
           (w w' : Σ[ x ∈ A ] (P x))
           → (w ≡ w') ≃ (Σ[ p ∈ (pr₁ w ≡ pr₁ w') ] (transport P p (pr₂ w) ≡ (pr₂ w')))
thm2-7-2 {_} {_} {A} {P} w w' = equiv-adjointify (path-sigma w w' , path-equiv-sigma w w' , hom₃ , hom₄ w w')

cor2-7-3 : {i j : Level} {A : Type i} {P : A → Type j}
           (z : Σ[ x ∈ A ] (P x))
           → z ≡ (pr₁ z , pr₂ z)
cor2-7-3 z = path-equiv-sigma z (pr₁ z , pr₂ z) (refl , refl)
